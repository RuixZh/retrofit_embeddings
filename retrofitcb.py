...
def retrofit(wordVec, newWordVecs, dim, syn_lexicon, ant_lexicon,  numIters=2, starting_alpha=1.0, lamda=0.5):
    ...
    for it in range(numIters):
        ...
        if (it+1) % 10 == 0:
            ...
            f_sl.write(str(it) + '\t' + str(round(sl_max + 0.00001, 3)) + '\t' + 'NS-sv-sumMax' + '\n')
            f_sl.write(str(it) + '\t' + str(round(sl_avg + 0.00001, 3)) + '\t' + 'NS-sv-sumMean' + '\n')
            print('simver_3500:\n')
            sv_max, sv_avg = evaluation.main(wordVec, eval_verb, dim)
            f_sv.write(str(it) + '\t' + str(round(sv_max + 0.00001, 3)) + '\t' + 'NS-sv-sumMax' + '\n')
            f_sv.write(str(it) + '\t' + str(round(sv_avg + 0.00001, 3)) + '\t' + 'NS-sv-sumMean' + '\n')
...
if __name__ == '__main__':
    ...
    wordVec, newWordVecs, dim = read_word_vecs(args.input)
    syn_lexicon = read_lexicon(args.syn_lexicon)
    ant_lexicon = read_lexicon(args.ant_lexicon)
    outFileName = args.output
    t0 = time.time()
    wordVec = retrofit(wordVec, newWordVecs, dim, syn_lexicon, ant_lexicon, args.numiter, args.alpha, args.lamda)
    t1 = time.time()
    print('\nCompleted retrofitting. Retrofitting took', (t1 - t0) / 60, 'minutes')
    print_word_vecs(wordVec, outFileName)
    '''

    syn_lexicon = read_lexicon('lexicon/wiki-syns.txt')
    ant_lexicon = read_lexicon('lexicon/wiki-ants.txt')

    input_file = 'word_vec/glove.6B.100d.txt'
    ...
    # wordVec_sv = retrofit(wordVecs, newWordVecs, dim, syn_lexicon, ant_lexicon, numIters=it)

    print('NS-sv-sum:\n')
    wordVec_sum = retrofit(wordVecs, newWordVecs, dim, syn_lexicon, ant_lexicon, numIters=it)
